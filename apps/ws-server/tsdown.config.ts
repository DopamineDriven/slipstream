import { arch, platform } from "node:os";
import { join, relative, resolve } from "node:path";
import type { UserConfig } from "tsdown";
import { defineConfig } from "tsdown";

const p = platform();
const a = arch();
const executable = p === "win32" ? "tsgo.exe" : "tsgo";
const dirname = `native-preview-${p}-${a}`;

const path = resolve(
  join(
    process.cwd(),
    `../../node_modules/@typescript/${dirname}/lib/${executable}`
  )
);

export default defineConfig(
  options =>
    ({
      ...options,
      entry: [
        "src/index.ts",
        "src/alibaba/index.ts",
        "src/alibaba/memory.ts",
        "src/alibaba/sse.ts",
        "src/alibaba/summarizer.ts",
        "src/alibaba/types.ts",
        "src/alibaba/workup.ts",
        "src/anthropic/base.ts",
        "src/anthropic/index.ts",
        "src/anthropic/summarizer.ts",
        "src/anthropic/types.ts",
        "src/anthropic/vector-store.ts",
        "src/anthropic/workup.ts",
        "src/byte-codec/index.ts",
        "src/cohere/index.ts",
        "src/cohere/types.ts",
        "src/deepseek/index.ts",
        "src/deepseek/memory.ts",
        "src/deepseek/sse.ts",
        "src/deepseek/summarizer.ts",
        "src/deepseek/types.ts",
        "src/deepseek/workup.ts",
        "src/extract/index.ts",
        "src/gemini/base.ts",
        "src/gemini/chat.ts",
        "src/gemini/ensure.ts",
        "src/gemini/fss.ts",
        "src/gemini/index.ts",
        "src/gemini/interactions-sse.ts",
        "src/gemini/interactions.ts",
        "src/gemini/types.ts",
        "src/gemini/workup.ts",
        "src/image/index.ts",
        "src/kimi/index.ts",
        "src/kimi/memory.ts",
        "src/kimi/sse.ts",
        "src/kimi/summarizer.ts",
        "src/kimi/types.ts",
        "src/kimi/workup.ts",
        "src/local-tools/local-tool-broker.ts",
        "src/local-tools/types.ts",
        "src/logger/index.ts",
        "src/memory/semaphore.ts",
        "src/memory/summarizer-loop.ts",
        "src/memory/types.ts",
        "src/memory/vector-store.ts",
        "src/memory/workup.ts",
        "src/meta/base.ts",
        "src/meta/chat.ts",
        "src/meta/index.ts",
        "src/meta/responses-image.ts",
        "src/meta/store.ts",
        "src/meta/types.ts",
        "src/meta/workup.ts",
        "src/minimax/index.ts",
        "src/minimax/memory.ts",
        "src/minimax/sse.ts",
        "src/minimax/summarizer.ts",
        "src/minimax/types.ts",
        "src/minimax/workup.ts",
        "src/mistral/index.ts",
        "src/mistral/memory.ts",
        "src/mistral/stream-content.ts",
        "src/mistral/types.ts",
        "src/mistral/workup.ts",
        "src/mixins/index.ts",
        "src/models/index.ts",
        "src/models/tool-defs.ts",
        "src/models/workup.ts",
        "src/openai/base.ts",
        "src/openai/gpt-image.ts",
        "src/openai/index.ts",
        "src/openai/memory.ts",
        "src/openai/responses-chat.ts",
        "src/openai/responses-img-gen.ts",
        "src/openai/summarizer.ts",
        "src/openai/types.ts",
        "src/openai/workup.ts",
        "src/pdf/index.ts",
        "src/prisma/attachment-provider.ts",
        "src/prisma/attachment.ts",
        "src/prisma/chat-request.ts",
        "src/prisma/chat-response.ts",
        "src/prisma/cli-config.ts",
        "src/prisma/convo-hydration.ts",
        "src/prisma/convo-list.ts",
        "src/prisma/convo-memory-service.ts",
        "src/prisma/index.ts",
        "src/prisma/provider-store.ts",
        "src/prisma/stt.ts",
        "src/prisma/tts.ts",
        "src/prisma/types.ts",
        "src/prisma/user-meta.ts",
        "src/prisma/user-store.ts",
        "src/prisma/utils.ts",
        "src/providers/index.ts",
        "src/resolver/asset-attach-or-paste.ts",
        "src/resolver/asset-compat.ts",
        "src/resolver/asset-complete.ts",
        "src/resolver/asset-fetch.ts",
        "src/resolver/chat-utils.ts",
        "src/resolver/chat.ts",
        "src/resolver/cli-config-hydrate.ts",
        "src/resolver/cli-config-update.ts",
        "src/resolver/cli-recent-convos.ts",
        "src/resolver/connection.ts",
        "src/resolver/convo-hydration.ts",
        "src/resolver/convo-list.ts",
        "src/resolver/dispatch.ts",
        "src/resolver/index.ts",
        "src/resolver/local-tool-result.ts",
        "src/resolver/stt.ts",
        "src/resolver/tts.ts",
        "src/resolver/utils.ts",
        "src/sakana/base.ts",
        "src/sakana/chat.ts",
        "src/sakana/index.ts",
        "src/sakana/store.ts",
        "src/sakana/types.ts",
        "src/sakana/workup.ts",
        "src/sharp/index.ts",
        "src/store/types.ts",
        "src/store/vector-store.ts",
        "src/store/workup.ts",
        "src/stt/index.ts",
        "src/tool-catalog/index.ts",
        "src/tool-catalog/types.ts",
        "src/tts/index.ts",
        "src/types/index.ts",
        "src/vercel/index.ts",
        "src/vercel/sse.ts",
        "src/vercel/types.ts",
        "src/voyage/index.ts",
        "src/voyage/types.ts",
        "src/ws-server/index.ts",
        "src/xai/api-methods.ts",
        "src/xai/api-workup.ts",
        "src/xai/base.ts",
        "src/xai/collections.ts",
        "src/xai/event-types.ts",
        "src/xai/img-gen.ts",
        "src/xai/index.ts",
        "src/xai/response-sse.ts",
        "src/xai/responses-api-linear.ts",
        "src/xai/responses-api.ts",
        "src/xai/responses-types.ts",
        "src/xai/stream-workup.ts",
        "src/xai/sync.ts",
        "src/xai/types.ts",
        "src/xai/user-store.ts",
        "src/zai/index.ts",
        "src/zai/memory.ts",
        "src/zai/sse.ts",
        "src/zai/summarizer.ts",
        "src/zai/types.ts",
        "src/zai/workup.ts",
        "!src/__out__/**/*",
        "!src/test/**/*",
        "!src/tests/**/*",
        "!public/**/*"
      ],
      cwd: process.cwd(),
      target: ["node26"],
      fixedExtension: false,
      dts: { tsgo: { path } },
      watch: process.env.NODE_ENV === "development",
      format: ["esm"],
      sourcemap: true,
      tsconfig: relative(process.cwd(), "tsconfig.json"),
      clean: true,
      outDir: "dist",
      unbundle: true
    }) satisfies UserConfig
);
