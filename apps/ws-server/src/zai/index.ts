...
export class ZaiService extends ZaiMemoryService {
  ...
  public async handleZaiAiChatRequest({
    ...
    const provider = "zai" as const satisfies Lowercase<$Enums.Provider>;
    ...
    if (localToolTurn) {
      this.logger.info(
        ...
        "local tool bridge armed for zai turn"
      );
    }
    let ZaiThinkingDuration = 0,
      ZaiThinkingAgg = "",
      ZaiAgg = "",
      totalUsage = 0;
    const trackedBlocks = Array.of<ZaiFinalizedMessageBlock>();
    let activeBlock: ZaiActiveMessageBlock | undefined = undefined;
    let nextOrdinal = 0;
    ...
    const finalizeActiveBlock = () => {
      ...
      if (activeBlock.type === "THINKING") {
        ZaiThinkingDuration += durationMs;
      }
    ...
    const ensureActiveBlock = (type: ZaiActiveMessageBlock["type"]) => {
    ...
    for (let round = 0; round <= MAX_TOOL_ROUNDS; round++) {
      const roundToolCalls = new Map<number, ZaiAccumulatedToolCall>();
      let roundUsage: ZaiUsage | undefined = undefined;
          ...
          if (isContentDelta(choice.delta)) {
            ...
            ws.send(
              JSON.stringify({
                ...
                temperature,
                thinkingDuration:
                  ZaiThinkingDuration > 0 ? ZaiThinkingDuration : undefined,
                isThinking: false,
                messageBlocks: currentChunkMessageBlock(),
                topP,
                model,
            ...
            void this.redis.publishTypedEvent(streamChannel, "ai_chat_chunk", {
              ...
              isThinking: false,
              thinkingText:
                ZaiThinkingAgg.length > 0 ? ZaiThinkingAgg : undefined,
              messageBlocks: currentChunkMessageBlock(),
              systemPrompt,
              temperature,
              topP,
      ...
      if (round === MAX_TOOL_ROUNDS) {
        ...
        this.logger.warn(
          ...
          "Zai tool loop reached max rounds"
        );
        break;
      }
      ...
      } as const satisfies ZaiAssistantToolCallMessage;
      ...
      this.logger.info(
        ...
        "Zai tool round complete, sending continuation"
      );
    }

    if (forcedLoopStopReason && ZaiAgg.trim().length === 0) {
      ZaiAgg =
    ...
    const d = await this.prisma.handleAiChatResponse({
      ...
      thinkingDuration:
        ZaiThinkingDuration > 0 ? ZaiThinkingDuration : undefined,
      thinkingText: ZaiThinkingAgg.length > 0 ? ZaiThinkingAgg : undefined,
