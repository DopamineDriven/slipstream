...
export class OpenAIResponsesImgGenService extends OpenAIGPTImageService {
  ...
  protected async handleOpenaiResponsesImgGen({
    ...
    // memory/user-store function tools mid-image-flow; each round re-streams
    // with function_call + output items appended
    for (let round = 0; round <= MAX_TOOL_ROUNDS; round++) {
      ...
      if (round === MAX_TOOL_ROUNDS) {
        this.logger.warn(
          { round, functionCallCount: functionCalls.length },
          "img-gen tool loop reached max rounds without a final image"
        );
        break;
      }
      const toolOutputs =
