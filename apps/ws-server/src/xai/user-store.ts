  ...
  protected slatherUserStore() {
    return {
      ...
      description:
        ...
        "When search_terms is provided, executes fulltext keyword search and returns " +
  ...
  protected async executeFunctionToolCall(
    ...
    try {
      if (toolName === "slather_user_store") {
        const input = this.userStore.parseUserStoreInput(toolCall.arguments, toolName);
        const output = await this.userStore.executeFileSearch(userId, input);
        return {
