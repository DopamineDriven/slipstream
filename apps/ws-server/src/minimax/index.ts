  ...
  public async handleMiniMaxAiChatRequest({
    ...
    if (localToolTurn) {
      this.logger.info(
        ...
        "local tool bridge armed for minimax turn"
      );
    }
    let MiniMaxThinkingDuration = 0,
      MiniMaxThinkingAgg = "",
      MiniMaxAgg = "",
      totalUsage = 0;
    const trackedBlocks = Array.of<MiniMaxFinalizedMessageBlock>();
    let activeBlock: MiniMaxActiveMessageBlock | undefined = undefined;
    let nextOrdinal = 0;
    ...
    const finalizeActiveBlock = () => {
      ...
      if (activeBlock.type === "THINKING") {
        MiniMaxThinkingDuration += durationMs;
      }
    ...
    const ensureActiveBlock = (type: MiniMaxActiveMessageBlock["type"]) => {
    ...
    const currentThinkingDuration = () => {
      ...
      return MiniMaxThinkingDuration + activeThinkingDuration;
    };
    ...
    const appendReasoningDelta = (reasoningText?: string) => {
      ...
      MiniMaxThinkingAgg += reasoningText;
    ...
    for (let round = 0; round <= MAX_TOOL_ROUNDS; round++) {
      const roundToolCalls = new Map<number, MiniMaxAccumulatedToolCall>();
      let roundUsage: MiniMaxUsage | undefined = undefined;
          ...
          if (isContentDelta(choice.delta)) {
            ...
            ws.send(
              JSON.stringify({
                ...
                temperature,
                thinkingDuration:
                  MiniMaxThinkingDuration > 0
                    ? MiniMaxThinkingDuration
                    : undefined,
                isThinking: false,
                messageBlocks: currentChunkMessageBlock(),
                topP,
            ...
            void this.redis.publishTypedEvent(streamChannel, "ai_chat_chunk", {
              ...
              isThinking: false,
              thinkingText:
                MiniMaxThinkingAgg.length > 0 ? MiniMaxThinkingAgg : undefined,
              messageBlocks: currentChunkMessageBlock(),
              systemPrompt,
              temperature,
              topP,
      ...
      if (round === MAX_TOOL_ROUNDS) {
        ...
        this.logger.warn(
          ...
          "MiniMax tool loop reached max rounds"
        );
        break;
      }
      ...
      } as const satisfies MiniMaxAssistantToolCallMessage;
      ...
      this.logger.info(
        ...
        "MiniMax tool round complete, sending continuation"
      );
    }

    if (forcedLoopStopReason && MiniMaxAgg.trim().length === 0) {
      MiniMaxAgg =
    ...
    const d = await this.prisma.handleAiChatResponse({
      ...
      systemPrompt,
      thinkingDuration:
        MiniMaxThinkingDuration > 0 ? MiniMaxThinkingDuration : undefined,
      thinkingText:
        MiniMaxThinkingAgg.length > 0 ? MiniMaxThinkingAgg : undefined,
