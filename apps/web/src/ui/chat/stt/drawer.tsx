      ...
      <AnimatePresence onExitComplete={finishClosing}>
        {open && (
          <div
            ...
            <motion.div
              aria-hidden="true"
              className="pointer-events-auto absolute inset-0 bg-[color-mix(in_srgb,var(--color-background)_48%,transparent)] [@media(width<=767px)]:bg-[color-mix(in_srgb,var(--color-background)_65%,transparent)]"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            <motion.div
              key="drawer-panel"
              className={cn(
                "bg-background text-foreground pointer-events-auto relative flex min-h-0 shadow-[-24px_0_70px_color-mix(in_srgb,var(--color-background)_32%,transparent)]",
                side === "bottom"
                  ...
                  : "h-full w-[min(var(--drawer-width,460px),100%)] border-l border-l-[color-mix(in_srgb,var(--color-foreground)_12%,transparent)] [@media(width<=767px)]:w-[min(var(--drawer-width,440px),100%)] [@media(width<=767px)]:border-l-0",
                className
              )}
              initial={offset}
