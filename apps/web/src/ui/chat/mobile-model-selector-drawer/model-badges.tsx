  ...
  return (
    <div className="flex w-full items-center justify-between">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {provider !=="gemini" && provider !=="grok" && model.includes("Preview") && (
          <Badge variant="secondary" className="h-5 px-1.5 py-0.5 text-xs">
            Preview
          </Badge>
        )}
        {provider !== "minimax" && model.includes("Mini") && (
      ...
      {isSelected && (
        <Check className="text-brand-primary-foreground ml-3 h-5 w-5 shrink-0" />
      )}
    </div>
  );
}
