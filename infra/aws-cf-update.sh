...
CERT_ARN=$(cat infra/remote/describe/acm/wildcard-aicoalesce-arn.txt)
REGION=us-east-1
...
update_distribution() {
    ...
    aws cloudfront get-distribution-config --id "$DIST_ID" \
      --output json > infra/remote/describe/cloudfront/${ENV}-current.json
    
    ETAG=$(jq -r '.ETag' infra/remote/describe/cloudfront/${ENV}-current.json)
    ...
    jq --arg cert "$CERT_ARN" --arg new_domain "$NEW_DOMAIN" --arg old_domain "$OLD_DOMAIN" '
      ...
      }' infra/remote/describe/cloudfront/${ENV}-current.json \
      | jq '.DistributionConfig' > infra/remote/describe/cloudfront/${ENV}-updated.json
    
    # Apply update
    aws cloudfront update-distribution \
      --id "$DIST_ID" \
      --distribution-config file://infra/remote/describe/cloudfront/${ENV}-updated.json \
      --if-match "$ETAG" \
      --output json > infra/remote/describe/cloudfront/${ENV}-result.json
    
    echo "✓ Updated $NEW_DOMAIN"
    
    # Save new config for your records
    cat > infra/remote/describe/cloudfront/${ENV}-aicoalesce-config.json <<EOF
{
  "environment": "$ENV",
  ...
  "distribution": {
    ...
    "domain": "$(jq -r '.DistributionConfig.DomainName' infra/remote/describe/cloudfront/${ENV}-current.json)"
  },
