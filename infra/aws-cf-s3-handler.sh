...
echo "ws-server-assets-dev" > "infra/oac-ws-server-assets-dev.txt" && echo "E1GNYMV79MQ3KS" > "infra/oac-ws-server-assets-dev.txt"
echo "py-gen-assets-dev" > "infra/oac-py-gen-assets-dev.txt" && echo "EJSAAPGQ9U9FS" > "infra/oac-py-gen-assets-dev.txt"
echo "ws-server-assets-prod" > "infra/oac-ws-server-assets-prod.txt" && echo "E77UWHQDD8CFH" > "infra/oac-ws-server-assets-prod.txt"
echo "py-gen-assets-prod" > "infra/oac-py-gen-assets-prod.txt" && echo "E1JW8URCO75ZVW" > "infra/oac-py-gen-assets-prod.txt"
...
create_distribution() {
    ...
    local OAC_UPLOAD=$(cat "infra/oac-${UPLOAD_BUCKET}.txt")
    local OAC_GENERATED=$(cat "infra/oac-${GENERATED_BUCKET}.txt")
    ...
    echo "$DIST_ARN" > "infra/dist-arn-${ENV}.txt"
    echo "$DIST_DOMAIN" > "infra/dist-domain-${ENV}.txt"
}
...
update_bucket_policy() {
    local BUCKET=$1
    local ENV=$2
    local DIST_ARN=$(cat "infra/dist-arn-${ENV}.txt")
    
    echo "Updating policy for $BUCKET..."
    
    cat > "infra/bucket-policy-${BUCKET}.json" <<EOF
{
    "Version": "2012-10-17",
    "Statement": [{
...
EOF
    ...
    aws s3api put-bucket-policy \
        --bucket "$BUCKET" \
        --policy file://infra/bucket-policy-${BUCKET}.json
}
...
if [[ "$UPDATE_CORS" == "y" ]]; then
    for BUCKET in "${DEV_BUCKETS[@]}" "${PROD_BUCKETS[@]}"; do
        ...
        cat > "infra/cors-${BUCKET}.json" <<EOF
{
    "CORSRules": [
        {
            "AllowedHeaders": ["*"],
...
EOF
        aws s3api put-bucket-cors --bucket "$BUCKET" --cors-configuration file://infra/cors-${BUCKET}.json
    done
fi
...
mkdir -p infra/remote/describe/acm

# Save production distribution info
PROD_DIST_DOMAIN=$(cat infra/dist-domain-prod.txt)
PROD_DIST_ARN=$(cat infra/dist-arn-prod.txt)
PROD_DIST_ID=$(echo "$PROD_DIST_ARN" | cut -d'/' -f2)

cat > infra/remote/describe/acm/cloudfront-prod.json <<EOF
{
...
DEV_DIST_DOMAIN=$(cat infra/dist-domain-dev.txt)
DEV_DIST_ARN=$(cat infra/dist-arn-dev.txt)
DEV_DIST_ID=$(echo "$DEV_DIST_ARN" | cut -d'/' -f2)

cat > infra/remote/describe/acm/cloudfront-dev.json <<EOF
{
...
cat > infra/remote/describe/acm/cloudfront-summary.json <<EOF
{
...
echo "Distribution info saved to infra/remote/describe/acm/"
echo ""
echo "==================================="
echo "CNAME records for DNS:"
echo "==================================="
cat infra/remote/describe/acm/cloudfront-summary.json | jq -r '.dns_records_to_add[]'