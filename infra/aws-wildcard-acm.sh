...
echo "$CERT_ARN" > infra/remote/describe/acm/wildcard-aicoalesce-arn.txt
...
aws acm describe-certificate \
  ...
  --region "$REGION" \
  --output json > infra/remote/describe/acm/wildcard-aicoalesce-validation.json

# Extract validation CNAME
VALIDATION=$(jq -r '.Certificate.DomainValidationOptions[0].ResourceRecord' infra/remote/describe/acm/wildcard-aicoalesce-validation.json)
...
aws route53 change-resource-record-sets \
  ...
  }" > infra/remote/describe/route53/validation-created.json
