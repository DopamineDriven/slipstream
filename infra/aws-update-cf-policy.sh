...
PROD_DIST_ID=$(cat infra/remote/describe/acm/cloudfront-prod.json | jq -r '.distribution.id')
DEV_DIST_ID=$(cat infra/remote/describe/acm/cloudfront-dev.json | jq -r '.distribution.id')
...
update_distribution_methods() {
    ...
    # Get current config
    aws cloudfront get-distribution-config --id "$DIST_ID" > "infra/dist-config-${ENV_NAME}.json"
    
    # Extract ETag (needed for update)
    ETAG=$(jq -r '.ETag' "infra/dist-config-${ENV_NAME}.json")
    ...
    }' "infra/dist-config-${ENV_NAME}.json" | jq '.DistributionConfig' > "infra/dist-config-${ENV_NAME}-updated.json"
    
    # Apply the update
    aws cloudfront update-distribution \
        --id "$DIST_ID" \
        --distribution-config file://infra/dist-config-${ENV_NAME}-updated.json \
        --if-match "$ETAG"
