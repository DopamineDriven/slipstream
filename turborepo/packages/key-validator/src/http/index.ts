...
// const xAi = process.env.X_AI_KEY ?? "";
...
//   [xAi, "grok"]
// ] as const;
// const helper = Array.of<[string, { isValid: boolean; message: string }]>();
// const Tester = async () =>
//   Array.from(tupled)
//     .map(([k, v]) => {
//       const test = new KeyValidator(k, v);
//       return test.validateProvider();
//     })
//     .map(async (res, i) => {
//       const response = await res;
//       console.log(response);
//       helper.push([
//         tupled?.[i as 0 | 1 | 2 | 3]?.["1"],
//         { isValid: response.isValid, message: response.message }
//       ]);
//     });
// Tester().then(_ => {
//   return _.map(t =>
//     t.then(_ => {
//       fs.withWs(
//         `src/test/__out__/init.json`,
//         JSON.stringify(Object.fromEntries(helper), null, 2)
//       );
//     })
//   );
// });
