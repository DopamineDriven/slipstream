...
import type { DocSpecs } from "@slipstream/types";
...
function toSafeString(buf: Uint8Array, encoding = "utf-8"): string {
  try {
...
function detectTextEncodingPrefix(buffer: Uint8Array): {
  encoding: string;
  offset: number;
} {
...
function _stripXmlTags(xml: string): string {
  return xml
    .replace(/<\/?[^>]+>/g, " ")
    .replace(/\s+/g, " ")
    .trim();
...
function firstN(text: string | null | undefined, n = 240): string | null {
  if (!text) return null;
...
function extToLanguage(ext: string | null): string | null {
  if (!ext) return null;
...
function parsePdf(buffer: Buffer, mime: string): DocSpecs {
...
function parseRtf(buffer: Buffer, mime: string): DocSpecs {
...
function parsePlainText(
  ...
  const { encoding, offset } = detectTextEncodingPrefix(u8);
  const text = toSafeString(u8.subarray(offset), encoding);
...
function readLocalFileData(buffer: Buffer, entry: ZipEntry): Uint8Array | null {
...
function parseOpenXml(
  ...
  const coreXml = core ? readLocalFileData(buffer, core) : null;
  const appXml = app ? readLocalFileData(buffer, app) : null;
  ...
  if (kind === "docx") {
    ...
    if (docEntry) {
      const data = readLocalFileData(buffer, docEntry);
      if (data) {
        const xml = toSafeString(data);
        ...
        const t = xml
          ...
          .replace(/\s+/g, " ")
          .trim();
        textPreview = firstN(t, 280);
        if (textPreview) {
          const words = countWords(textPreview);
          wordCount ??= words;
        }
      }
    }
...
export class DocMetadataExtractor {
  public getDocumentSpecsWorkup(
        ...
        (ext === "docx"
          ...
          : ext === "pptx"
            ...
            : ext === "xlsx"
              ...
              : undefined);
      if (kind) {
        return parseOpenXml(buffer, mime, kind);
      }
    }
    ...
    if (
      ...
      return parsePlainText(buffer, mime, filename);
    }
