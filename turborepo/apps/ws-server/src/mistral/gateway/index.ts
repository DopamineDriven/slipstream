...
import type {
  MistralChatCompletionsRes,
  MistralUsage
} from "@/mistral/gateway/sse.ts";
import type { Logger as PinoLogger } from "pino";
import { LoggerService } from "@/logger/index.ts";
import { PrismaService } from "@/prisma/index.ts";
import {
  createMistralSSEParser,
  hasToolCallDelta,
  isContentDelta,
  isReasoningDelta
} from "@/mistral/gateway/sse.ts";
...
type MistralAssistantToolCallMessage = {
  role: "assistant";
  content: "";
  tool_calls: readonly MistralFunctionToolCall[];
};
...
export class MistralGatewayService {
  ...
  constructor(
    ...
    this.logger = logger
      ...
      .child(
        ...
        { msgPrefix: "[mistral-gateway] " }
      );
  }

  private async *stream(
    model = "mistral-small-latest" satisfies MistralModelIdUnion,
    messages: readonly MistralRequestMessage[],
    apiKey?: string,
    options?: {
      temperature?: number;
      top_p?: number;
  ...
  ): AsyncGenerator<MistralChatCompletionsRes, void, unknown> {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Mistral AI Gateway error (${response.status}, ${response.statusText}): ${errorText}`
      );
    }

    const parser = createMistralSSEParser(response);
  ...
  private formatMsgs(
    ...
    return [
      ...
      ...msgs
    ] satisfies MistralRequestMessage[];
  }
  ...
  public async handleMistralGatewayChatRequest({
    chunks,
    conversationId,
    streamChannel,
    msgs,
    ...
    let roundMessages = Array.of<MistralRequestMessage>(
      ...this.mistralFormat(isNewChat, msgs, systemPrompt, hasUserStoreDocs)
    );
    ...
    for (let round = 0; round <= MAX_TOOL_ROUNDS; round++) {
      ...
      let roundUsage: MistralUsage | undefined = undefined;
          ...
          if (isContentDelta(choice.delta)) {
            ...
            ws.send(
              JSON.stringify({
                ...
                temperature,
                thinkingDuration:
                  mistralThinkingDuration > 0
                    ? mistralThinkingDuration
                    : undefined,
                isThinking: false,
                messageBlocks: currentChunkMessageBlock(),
                topP,
      ...
      roundMessages = Array.of<MistralRequestMessage>(
        ...roundMessages,
