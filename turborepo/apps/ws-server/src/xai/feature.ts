...
export class xAIFeatureService {
  ...
  public xAiFormat(
    ...
    userPrompt: ProviderChatRequestEntity["prompt"],
    ...
    | { role: "user"; content: string | readonly (
        | { type: "text"; text: string }
        | { type: "image_url"; image_url: { url: string; detail?: "low" | "medium" | "high" } }
      )[] }
    | { role: "assistant"; content: string }
  )[] {
    // Build current-turn user message with optional image_url parts
    ...
    if (isNewChat) {
      if (systemPrompt) {
        return [
          ...
          { role: "user", content: currentContent.length === 1 && currentContent[0]?.type === "text"
            ? (currentContent[0] as { type: "text"; text: string }).text
            : currentContent }
        ] as const;
      } else {
        return [{
      ...
      // For existing chats, `msgs` already includes the latest user message
      // (synced from the database). Avoid appending the current `userPrompt`
      // again to prevent duplicate user turns.
      const history = this.prependProviderModelTag(msgs);
      const formatted = this.formatMsgs(history, systemPrompt);
      return formatted;
    }
  }
