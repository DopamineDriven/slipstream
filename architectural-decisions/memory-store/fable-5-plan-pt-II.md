...
- **Assembly is substitution, not compaction.** Message ranges between the two verbatim anchors are replaced in each provider's history by their section summaries — name-tagged to the summarizing model via the platform's `[PROVIDER/MODEL]` convention, ordinal-prefixed (`[0-12]`, `[96-107]`), gap-tolerant. Verbatim at both ends — a 30-message **founding window** (the charter anchor, §2.4) and a 30-message **live window** (recency) — with consolidated traces bridging the middle: the serial-position shape. Models reach back for firsthand transcripts on demand via tools. We do **not** rely on any provider's native compaction; most don't have it and it isn't the point (§2).
- **The indexing horizon collapses to 0.** Indexing chases the conversation tip; the DP sectioner's `minSectionTokens` band is the natural "not enough for a decent chunk yet" holdback. Recency (and primacy) staging lives where it belongs — at history assembly (`foundingWindowMessages`/`liveWindowMessages`), not indexing (§1).
...
- **Verbatim recent experience** — the live window (newest 30 messages), rendered raw in provider history exactly as it always has been — and **verbatim origins**: the founding window (oldest 30), the conversation's charter (§2.4).
...
- **Part I's compaction machinery: DELETED, not renamed** (done pre-Phase-1, per Andrew). The anthropic formatter's plan fetch/ordinal-skip/leading-block unshift, `getCompactionPlan`, `memoryCompactionConfig`, `MemoryCompactionConfig`, and `findCompactableChunks` are all removed — the contiguous-prefix design isn't what §2 builds, so Phase 2 writes `memoryAssemblyConfig` and `getHistoryAssemblyPlan` fresh from the substitution design, with a finder that carries `summaryModel`/`summaryProvider` for name tags. (Settled at `foundingWindowMessages: 30` + `liveWindowMessages: 30` — §2.4; effective verbatim recency ≈ `30 + summary lag (~12–20 messages)`, and the charter end stays verbatim permanently.)
...
### 2.3 Role alternation — probed, and the "hard constraint" fell
...
**The leading-`user` requirement is a myth in the current API — probed 2026-07-05.** Four probes against `/v1/messages`: user-first control (200), assistant-first with a name-tagged summary block (200, clean completion), consecutive assistant summaries mid-history (200), and assistant-first under the **full production posture** — sonnet-5, adaptive thinking, `output_config`, all six betas, server tools (200, clean completion). The constraint belonged to an older API generation; the stale skill citation is retired, along with the anonymous leading-user stub it justified. (Ending on assistant remains unprobed — structurally unreachable: history always ends on the current user message.) In practice the founding window (§2.5) opens history with genuine user voice anyway.

### 2.4 The founding window — primacy is real, infantile amnesia is not

Ordinals `[0, 30)` always render verbatim — the **charter anchor**, symmetric with the 30-message live window. The reasoning, in order of force:

1. **The human-memory analogy breaks in favor of preservation.** Humans forget their origins because the encoding machinery was immature at the time — no language, no self-schema. An agent at ordinal 0 is at full acuity; the founding exchanges are encoded as richly as anything after, and message 0 is routinely the conversation's highest-information-density message (the task, the constraints, the register — the charter).
2. **The provider carousel.** On a 12-provider platform, an opening question carouseled across the fleet is `[0-23]` before the conversation proper starts (common in practice). A narrow founding window would truncate mid-carousel; 30 covers a full carousel plus follow-through.
3. **Serial-position + lost-in-the-middle.** Human recall is vivid at both ends of an episode and gist in the middle; transformer attention empirically sags in the middle of context and peaks at the ends. Both point at the same shape: verbatim at the top (premium attention real estate spent on charter, not summary-of-charter), verbatim at the bottom, consolidated traces bridging — HMEM as the canonical memory curve.
4. **Register survives.** Summaries preserve facts but flatten voice; the founding block preserves how the conversation *talks* at the position models attend to hardest.

Both knobs (`foundingWindowMessages: 30`, `liveWindowMessages: 30`) are config; the `summaryToolUseRaw` captures close the tuning loop — frequent `get_chunk` fetches of early sections would signal the charter is too thin. Sections entirely inside the founding window never substitute (verbatim wins); sections straddling the boundary render their founding-side ordinals verbatim *and* emit their summary (trivial, honest overlap — the range prefix says exactly what's covered).
...
**One arm wired in Phase 5 (rescoped 2026-07-05, Andrew): GPT-5.5 only.**

- **GPT-5.5** (OpenAI Responses) — a genuinely exceptional summarizer, exceptionally token-efficient; the Responses surface is standardized in OpenAI's SDK, so the arm is anthropic-shaped work (simpler, even — no signed-thinking round-trips). `xhigh` effort, no output limit, ≥10 tool rounds.
- **grok-4.20 arm → filed to Part III.** The rationale was never wrong (cheapest strong output, most-typed provider) but the *service* surface is the largest in the codebase — 14 hand-rolled services with full feature sets — so an arm there is days of delicate threading, not hours. Arms are additive: router/envelope/usage-capture are arm-count-agnostic, so grok joins later without rework — IF the sonnet-5/GPT-5.5 audition data says the cost spread still wants it.

GPT-5.5 auditions per-chunk against sonnet-5; usage/quality receipts on every row decide rotation weights empirically.
...
- **Persist-time wrapper sanitizer — built, then deliberately NOT operationalized (Andrew, 2026-07-05).** `scrubModelWrappers` lives in `models/workup.ts` as a utility, but the persist-boundary call sites were reverted: with the name-tag normalization removing the mimicry *source* (formatter emits brackets; dev Claude messages confirmed bracket-clean) and the dev scrub having cleared the corpus, a hot-path regex pass on every AI persist would guard against a cause that no longer exists. Prod stays clean post-scrub for the same reason. (`scrub-model-wrappers.ts` proved the original exposure: 103k chars across 32 dev conversations, cross-provider mimicry — ANTHROPIC 355, GROK 13, GEMINI 4, OPENAI 2, MISTRAL/META 1 each.)
...
| **2** | §2 substitution assembly — fresh `memoryAssemblyConfig` (founding 30 / live 30) + `getHistoryAssemblyPlan`, name-tagged, gap-tolerant, dialect-aware — **Anthropic formatter first**, then mechanical fleet rollout | The core Part II feature. Anthropic first, then the 13 mechanical ports retire the 175-slice hack. | `memory/vector-store.ts` + 14 provider formatters |
...
| **5** | §6 MoE — **GPT-5.5 arm** (rescoped from grok+GPT, see §6.4) + router + `setProviders` + per-call usage capture; then the beast test under the two-arm mixture | Benefits from everything above being stable. GPT-5.5 rides OpenAI's standardized SDK — anthropic-shaped work; grok's arm filed to Part III (14 hand-rolled services = days not hours). The beast test doubles as the sonnet-5/GPT-5.5 audition. | new `summarizer-arms/*` (openai-responses arm), `vector-store.ts` router, `index.ts`, possible usage-column migration |
...
- **pdfdown-output-as-tool → "document vision by coordinates" (Part III major target, per Andrew 2026-07-05)** — give the no-native-document providers (deepseek, kimi, qwen, glm, minimax) document reading without pixels: `fs.fetchRemoteWriteLocalLargeFile(targetUrl, sourceUrl)` + pdfdown strip PDFs in milliseconds (721 pages/295ms), served as a **page-addressable tool** — fetch page N verbatim with per-page bounding boxes (whole page + annots + images), so models sift by page number and reason spatially over layout without OCR-through-vision. Same doctrine as HMEM: `file_search` finds *which* document (lay of the land), the page reader walks it (dig in and expand) — a natural `pairsWith` row in the catalog. Killer app is the five text-only providers, but it benefits the whole fleet: text+layout is cheaper and more precise than page images even for vision models.
...
- **Resolver-level ordinal sort (Part III seed)** — sort `res.messages` once in `resolver/chat.ts` instead of per-formatter (kimi/mistral/alibaba each re-sort today, a pre-existing slice-helper behavior). Deferred per Andrew: centralization of that kind belongs to a Phase III pass, not the substitution rollout.
