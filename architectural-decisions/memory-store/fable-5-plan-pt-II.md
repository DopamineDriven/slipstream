...
- **`liveWindowMessages: 20 → 50`** (`memoryCompactionConfig`, to be renamed — see below). This is now the *one* recency knob: the newest 50 ordinals render verbatim in every provider's history; summaries stand in below that. Effective verbatim context ≈ `liveWindow (50) + summary lag (~12–20 messages + minutes of latency)`, matching the "50–70 verbatim" preference.
- **Rename `memoryCompactionConfig` → `memoryAssemblyConfig`** (and `getCompactionPlan` → `getHistoryAssemblyPlan`, `compactedThroughOrdinalExclusive` → `substitutedThroughOrdinalExclusive`) to match §0.1. `enabled` and `liveWindowMessages` fields carry over; behavior is unchanged by the rename.
...
|---|---|---|---|
| **1** | §1 realignment (horizon→0, liveWindow→50, config rename) + §3 fold completion (tail + budget gate) | Pure config + fold-body; no schema, no provider surface. Fixes the observed over-indexing and the fold's tail-hunting immediately. | `memory/{workup,vector-store,types}.ts` |
| **2** | §2 substitution assembly — name-tagged, gap-tolerant, dialect-aware — **Anthropic formatter first**, then mechanical fleet rollout | The core Part II feature; §1's liveWindow decision propagates here. Anthropic first (already has an assembly path), then the 13 mechanical ports retire the 175-slice hack. | `memory/vector-store.ts` + 14 provider formatters |
...
| **5** | §6 MoE — **grok-4.20 + GPT-5.5 arms** + router + `setProviders` + per-call usage capture; then the 600-message beast test under the mixture | The largest change; benefits from everything above being stable. Both arms land together (sonnet-5 stays); the beast test is the real validation and the three-way summarizer audition in one. | new `summarizer-arms/*` (grok + openai-responses arms), `vector-store.ts` router, `index.ts`, possible usage-column migration |
