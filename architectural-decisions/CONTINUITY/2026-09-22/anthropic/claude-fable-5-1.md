...
- **One THINKING block from image `added` to the CDN url (Andrew, 09-22).** On `added`:
  ...
  close THINKING with ONE duration (added → url) → push `IMAGE_GEN` with `attachments: [..]`
...
| C | 2+3: `ChatChunkAndResMsgBlock.attachments?: AIChatResponseImgGenSubFields[]`; `AIChatResponseImgGenSubFields` gains `inlineImageGenOutput`; persist: two-phase (`updateMany` attachments' `messageBlockId` by `s3ObjectId` after the message create), nested `inlineImageGenOutput: { create }` on the attachment like `imageGenOutput` today; `isImageGen` stays false for one-offs; `Usage` type gains `cost_in_usd_ticks`, `image_generation_calls`, `context_details` | not started |
...
| D | 4: the image branches IN `responses-api-linear.ts` — plan step 4 has the site-by-site table (closeBeforeEvent guards, added → open THINKING, delta → heldText, done → (1)–(6), persist flags). Handle MULTIPLE image items per request (chaining). Needs C first (`attachments` on the wire block type) | not started |
